...
public:
    void showA(A x) {             // Public method that takes an object of type A
        std::cout << "A::a=" << x.a; // ERROR: Cannot access 'a' of another A object, even in derived class
    }
};
...
int main() {
   ...
   b.showA(a);                   // Attempt to display 'a' of object 'a' using B's showA (will cause compile error)
